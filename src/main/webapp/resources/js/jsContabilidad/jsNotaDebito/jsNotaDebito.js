...
$("#btnBuscarNotaDebito").click(function () {
    ...
    if (criterio === "2") {
        nroDoc = $("#txtBusquedaNroNotaDebito").val();
        fecIni = '';
        fecFin = '';
    } else {
        nroDoc = '';
    ...
    buscarNotasDebito(nroDoc, fecIni, fecFin);
});
...
$("#btnRegistrarNotaDebito").click(function () {

    bootbox.confirm("\u00BFEst\u00E1 seguro que desea registrar la nota de d\u00E9bito\u003F", function (result) {
        if (result) {
...
$('#tabNotaDebito a').click(function (e) {
    e.preventDefault();
    $(this).tab('show');
});
...
function buscarNotasDebito(nroDoc, fecIni, fecFin) {
    var data = {
        'tipo': '0008',
        'nroDoc': nroDoc,
        'fecIni': fecIni,
    ...
    var success = function (res) {
        if (res.Result === 'OK') {
            ...
            listarTablaNotaDebito(listaDocCompraVenta);
        } else {
...
function obtenerFactura(nroDoc, idDoc, idOficina, tipMoneda) {
    ...
    fn_callmethod("../sNotaDebito?action=obtenerDocumentoGeneradoNota", data, success, error);
}
...
function registrarDocumento(idTipoNota, detalleMotivo, fechaDoc) {
    ...
    var success = function (res) {
        ...
        if (res.Result === 'OK') {
            ...
            toast('success', "SE REGISTRO LA NOTA DE DEBITO DE MANERA EXITOSA", 'Exito!');
...
function listarTipos() {
    ...
    var success = function (res) {
        if (res.Result === 'OK') {
            let listaTipos = res.listarTipoNotaDebito;
            listarTipoNotaDebito(listaTipos);
        } else {
            toast('error', res.mensaje, 'Error');
        }
    };
    ...
    fn_callmethod("../sNotaDebito?action=listarTipoNotaDebito", '', success, error);
}
...
function listarTipoNotaDebito(listaTipos) {
    $('#cboTipo').empty();
