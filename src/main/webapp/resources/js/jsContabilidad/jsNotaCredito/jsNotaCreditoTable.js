function listarTablaNotaCredito(listaDocCompraVenta) {
    $('#tblNotasCredito').DataTable({
        ...
        "columns": [
            ...
            {"data": null,
                "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                    $(nTd).html("<a href = \"../sDownload?idFact=07-" + sData.documentoNro+"\" class='btn btn-Lapislazuli btn-circle xml' title='Descargar XML'><i class='fa fa-file-code-o'></i></a>");
                },
            className: "text-center", "width": "10%"},
            {"data": null,
                "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                    $(nTd).html("<a  href = \"../sDownloadCDR?idFactCDR=07-" + sData.documentoNro+"\" class='btn btn-Lapislazuli btn-circle cdr' title='Descargar CDR'><i class='fa fa-cloud-download'></i></a>");
                },
            className: "text-center", "width": "10%"}
...
$("#tblNotasCredito").on("click",".print",function(){
    fn_imprimirFactura(this);
})

$("#tblNotasCredito").on("click",".info",function(){
    fn_infoFactura(this);
})
...
function fn_infoFactura(elem){
    ...
    var nroFactura= par.children("td:nth-child(1)").html();
        ...
        fn_callmethod("../sNotaCredito?action=obtenerRsptaSUNAT", data, success, error);
}
