...
$(function () {
    ...
    oTable.on("click", ".remove", function () {
        var elem = this;
        console.log(elem);
        bootbox.confirm("\u00BFEst\u00e1 seguro de eliminar esta ocupacion?\u003F", function (result) {
            if (result) {
                oTable.fnDestroy();
                fn_Delete(elem);
            }
...
var fn_save = function (elem) {
    ...
    if (result === true)
    {
        ...
        fn_callmethod(base_url + "../sOcupacion?action=registrar", data, success, error);
...
var fn_Delete = function (elem) {
    ...
    if (codigo !== 0) {
        ...
        fn_callmethod(base_url + "../sOcupacion?action=delete", {codigo: codigo}, success, error);
    }
};
