...
import ll.comprobantes.electronicos.doc.DespatchAdviceUBL21;
 ...
 * @author RenRio
 */
public class sGuiaRemision extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (action.equalsIgnoreCase("buscarDocumentoGuiaRemision")) {
                buscarDocumentoGuiaRemision();
            }
    ...
    private void responseSUNAT() throws IOException {
        ...
        objGuiaRemisionxml.setNroGuia(varRequest.getParameter("nroFactura"));

        objGuiaRemisionxml = ReadXML.getResultGRM("A:\\DocumentosElectronicos\\DespatchAdvice\\20315295573-09-" + objGuiaRemisionxml.getNroGuia() + "\\R-20315295573-09-" + objGuiaRemisionxml.getNroGuia() + ".xml", objGuiaRemisionxml);

        dataObject.put("Result", "OK");
        dataObject.put("response", objGuiaRemisionxml.getResponse());
        json = gson.toJson(dataObject);
        varResponse.getWriter().write(json);
    }
    ...
    private void registrarGuia() throws Exception {
        ...
        /*
        'certificado': $("#txtCertificado").val(),
        ...
        objDocumentoGenerado.setIdDoc("0005");
...
//        ArrayList<HashMap<String, String>> detalleGM = (ArrayList<HashMap<String, String>>) sesion.getAttribute("detalladoGuiaRemision");
        objDocumentoGenerado.setObjOficina(objUsuario.getObjEmpleado().getObjOficina());
        HashMap<String, String> result = new HashMap<>();

        result = GuiaRemisionNEG.Instancia().registrarGuiaRemision(objDocumentoGenerado, objGuiaRemision, objUsuario, detalleGM);

        Boolean band = ParametroDAO.Instancia().actualizarParametroGuiaRemisionElectronica(objGuiaRemision.getNroGuia(), objDocumentoGenerado.getNroDoc(), objUsuario.getObjEmpleado().getObjOficina().getIdOficina(), objDocumentoGenerado.getObjTipoMoneda().getId());

        objGuiaRemisionxml = MySQLDocumentoCompraVentaNEG.Instancia().obtenerGuiaRemisionXML(objDocumentoGenerado);
        dataObject.put("nroDoc", objGuiaRemisionxml.getNroDoc());
        dataObject.put("idDoc", objGuiaRemisionxml.getIdDoc());
        dataObject.put("idOficina", objGuiaRemisionxml.getIdOfi());
        dataObject.put("tipMoneda", objGuiaRemisionxml.getTipM());

        DespatchAdviceUBL21.Instancia().c_XML(objGuiaRemisionxml);
    ...
    private void buscarGuiaRemision() throws Exception {
        ...
        List<ListaDocumentoCompraVenta> listaDetDocCompraVenta = GuiaRemisionNEG.Instancia().buscarGuiaRemision(criterio, valor);
    ...
    private void buscarDocumentoGuiaRemision() throws Exception {
        ...
        List<ListaDocumentoCompraVenta> listaDetDocCompraVenta = GuiaRemisionNEG.Instancia().buscarDocumentoGuiaRemision(nroDoc, fecIni, fecFin);
